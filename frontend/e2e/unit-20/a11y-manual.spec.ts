...
test("Tab 순서(데스크톱, 패널 열림): 툴바(탭→웹캠) → 채팅 → 패널, 숨김 패널에는 도달하지 않는다", async ({ page }) => {
  const room = await createRoom("live");
  await openRoom(page, room);
  const seq: string[] = [];
  for (let i = 0; i < 9; i++) {
    await page.keyboard.press("Tab");
    seq.push(await page.evaluate(() => (document.activeElement?.getAttribute("aria-label") ?? document.activeElement?.textContent ?? document.activeElement?.tagName ?? "").trim().slice(0, 24)));
  }
  expect(seq.slice(0, 3)).toEqual([TAB_CODE, TAB_WB, "웹캠 미리보기 켜기"]);
  expect(seq.join("|")).not.toContain("채팅으로 돌아가기"); // 닫힌 패널(display:none)은 포커스 불가
});
