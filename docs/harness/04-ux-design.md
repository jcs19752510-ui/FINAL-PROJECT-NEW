...
#### [C-11]/[R-02] 공통 추가 — 루브릭 항목별 평가 (v3 보정, 03-system-design v4 §4.6, DEC-055)
- **목적**: REQ-010(템플릿 기반 점수)과 REQ-012(어떤 답변에 어떤 기준이 적용됐는지)를 화면에서 확인하게 한다. v0~v2의 "평가 근거 아코디언"이 실제로 구현되지 않았던 공백(unit-11 DEF-002)을 이 명세로 구체화한다.
- **위치**: "세부 점수" 아래, STAR 본문 위. 섹션 제목 "루브릭 항목별 평가", 부제에 템플릿 이름(예: "기본 루브릭").
- **항목 행(템플릿 항목 수만큼)**: 항목명 · 가중치(%) · 점수 "n / 5"(없으면 "-" + "평가 근거 부족") · 근거 문장 · "근거 답변" 칩(`답변 #N`).
  - 칩을 누르면 해당 답변 발췌(`answers[].excerpt`, 최대 120자)를 바로 아래에 펼친다. `<button aria-expanded>`로 키보드 조작이 되게 하고, 모바일에서는 전체 폭으로 표시한다.
- **빈 상태**: `rubric=null`(이 기능 이전 리포트, 또는 파싱 실패 폴백)이면 "세부 근거 없음 — 이 리포트는 항목별 평가 없이 생성되었습니다"를 한 줄로 표시한다(`RubricEvidenceAccordion` empty variant).
- **보안**: 근거·발췌·템플릿 설명은 모두 JSX 텍스트로만 렌더링한다(REQ-036, HTML 해석 금지).
- **[R-02] 전용 — 채점 템플릿 변경**: 섹션 상단에 템플릿 선택 드롭다운(시스템 기본 + 내 템플릿, `GET /recruiter/rubric-templates`)과 "이 템플릿으로 다시 채점" 버튼.
  - 브라우저 `confirm()`을 쓰지 않고, 화면 안 확인 문구로 처리한다: "리포트가 새 기준으로 다시 생성되며 지원자 화면에도 반영됩니다. [다시 채점] [취소]".
  - 응답별 처리: 202이면 "채점 중" 상태로 전환하고 폴링한다. 200(변경 없음 또는 면접 진행 전)이면 "적용됨" 안내를 띄운다. 409면 "이미 채점이 진행 중입니다" 인라인 에러를 띄운다. 503(`QUEUE_FULL`)이면 "지금 처리 대기 중인 작업이 많습니다. 잠시 후 다시 시도해 주세요"를 띄운다.
  - 지원자 화면([C-11])에는 이 컨트롤이 없다.
  - 현재 적용된 템플릿이 동료 recruiter 소유라 선택 목록에 없으면, 드롭다운 위에 "현재: {스냅샷 이름}(다른 담당자 템플릿)"을 표시한다.
  - 근거 답변 칩의 원문이 삭제된 경우(`answers`에 없음)에는 칩을 비활성화하고 "원문이 삭제됨"을 표시한다.
...
| 2026-09-23 | **v3 (03-system-design v4 §4.6 반영, DEC-055)** | [C-11]/[R-02] 공통 "루브릭 항목별 평가" 섹션 명세 추가(항목·가중치·점수·근거·근거 답변 칩과 발췌, 빈 상태, [R-02] 채점 템플릿 변경 컨트롤과 화면 내 확인) | unit-11 DEF-002(평가 근거 화면 미노출)와 REQ-010 템플릿 기반 채점의 화면 요구 구체화. 검증 로그 `verify-log_04-ux-design.md` v3 절 |
